...
public class PlayerDAO {
        ...
        try {
           ...
           List<Boolean> list = new ArrayList<>(Arrays.asList(new Boolean[Manager.ACHIEVEMENTS.size()]));
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, data_card, bill_data, info_phoban, data_may_do, info_achievement,data_cai_trang_send,data_binh_can) "
                    ...
                    itemTime, itemTimeSC, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, data_card, bill_data, info_phoban, itemMayDo, info_achive,dkhi , itemBinhCan);
            ...
            try {
                ...
                if (player.isDie()) {
                    ...
                    if (MapService.gI().isMapDoanhTrai(mapId) || MapService.gI().isMapBlackBallWar(mapId)
                            || MapService.gI().isMapBanDoKhoBau(mapId) || MapService.gI().isMapMaBu(mapId) || MapService.gI().isMapConDuongRanDoc(mapId) || MapService.gI().isnguhs(mapId) || MapService.gI().isMapConDuongRanDoc(mapId)) {
                        mapId = player.gender + 21;
                        x = 300;
                        y = 336;
                    }
                ...
                //data duoi khi
                            ...
                            dataArray.clear();
                //data item may dò
                dataArray.add((player.itemTime.isUseMaydoDaPs ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() - player.itemTime.lastTimeMaydoDaPs)) : 0));
                ...
                String query = " update player set dhtime = ?, dhtime2 = ?, dhtime3 = ?, dhtime4 = ?, dhieu = ?,capcs = ?,capTT = ?, info_achievement =?, data_may_do = ?, khi_gas =?, data_item_time_sieu_cap = ?,head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, pointPvp=?, NguHanhSonPoint=?,data_card=?,bill_data =? ,info_phoban =? ,data_cai_trang_send =? ,data_binh_can =? where id = ?";
    ...
    public static boolean vnd(Player player, int num) {
        PreparedStatement ps = null;
        try (Connection con = GirlkunDB.getConnection();) {
