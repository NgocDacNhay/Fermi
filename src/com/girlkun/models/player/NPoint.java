...
public class NPoint {
    ...
    private void setHpMax() {
        ...
        //pet broly
        if (this.player.isPet && ((Pet) this.player).typePet == 18// chi so lam sao bac tu cho dj
                && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2 || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA3 || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA4)) {
...
//        }
    ...
    private void setMpMax() {
        ...
        //pet broly
        if (this.player.isPet && ((Pet) this.player).typePet == 18
                && (((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2 || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA3 || ((Pet) this.player).master.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA4)) {
...
//        }
    ...
    public long calSubTNSM(long tiemNang) {
        if (power >= 1000000000L) {
            ...
            tiemNang -= ((long) tiemNang * 55 / 100);
        } else if (power >= 150000000000L) {
            tiemNang -= ((long) tiemNang * 57 / 100);
        } else if (power >= 200000000000L) {
            tiemNang -= ((long) tiemNang * 59 / 100);
        } else if (power >= 300000000000L) {
            tiemNang -= ((long) tiemNang * 61 / 100);
        } else if (power >= 400000000000L) {
            tiemNang -= ((long) tiemNang * 63 / 100);
...
//        System.err.println("Hp còn lại " + this.hp);
    ...
    public long getPowerLimit() {
        switch (limitPower) {
            ...
            case 11:
                return 330999999999L;
            case 12:
                return 370999999999L;
            case 13:
                return 400999999999L;
            case 14:
                return 440999999999L;
            case 15:
                return 470999999999L;
            case 16:
    ...
    public void increasePoint(byte type, short point) {
        ...
        long tiemNangUse = 0;
        int heSo = 1;
            if(this.tiemNang < 100000){
                heSo = 1;
            } else if(this.tiemNang < 1000000){
                heSo = 10;
            } else if(this.tiemNang < 10000000){
                heSo = 20;
            } else if(this.tiemNang < 100000000){
                heSo = 30;
            } else if(this.tiemNang < 1000000000){
                heSo = 50;
            } else if(this.tiemNang < 10000000000L){
                heSo = 100;
            } else{
                heSo = 1000;
            }        
        if (type == 0) {
            int pointHp = heSo*point * 20;
            tiemNangUse = heSo*point * (2 * (this.hpg + 1000) + pointHp - 20) / 2;
            if ((this.hpg + pointHp) <= getHpMpLimit()) {
                if (doUseTiemNang(tiemNangUse)) {
                    hpg += pointHp;
                }
            } else {
                if(this.hpg < getHpMpLimit() && (this.hpg + pointHp) > getHpMpLimit()){
                    tiemNangUse = (getHpMpLimit() - this.hpg) * (2 * (this.hpg + 1000) + pointHp - 20) / 2;
                    if (doUseTiemNang(tiemNangUse)) {
                        hpg = getHpMpLimit();
                    }
                }else{
        ...
        if (type == 1) {
            int pointMp = heSo*point * 20;
            tiemNangUse = heSo*point * (2 * (this.mpg + 1000) + pointMp - 20) / 2;
            if ((this.mpg + pointMp) <= getHpMpLimit()) {
                if (doUseTiemNang(tiemNangUse)) {
                    mpg += pointMp;
                }
            } else {
                if(this.mpg < getHpMpLimit() && (this.mpg + pointMp) > getHpMpLimit()){
                    tiemNangUse = (getHpMpLimit() - this.mpg) * (2 * (this.mpg + 1000) + pointMp - 20) / 2;
                    if (doUseTiemNang(tiemNangUse)) {
                        mpg = getHpMpLimit();
                    }
                }else{
        ...
        if (type == 2) {
            int pointDame = heSo*point;
            tiemNangUse = heSo*point * (2 * this.dameg + pointDame - 1) / 2 * 100;
            if ((this.dameg + pointDame) <= getDameLimit()) {
                if (doUseTiemNang(tiemNangUse)) {
                    dameg += pointDame;
                }
            } else {
                if(this.dameg < getDameLimit() && (this.dameg + pointDame) > getDameLimit()){
                    tiemNangUse = (getDameLimit() - this.dameg) * (2 * this.dameg + pointDame - 1) / 2 * 100;
                    if (doUseTiemNang(tiemNangUse)) {
                        dameg = getDameLimit();
                    }
                }else{
    ...
    public void increasePointForPet(byte type, short point) {
        if (point <= 0 || point > 100) {
            return;
        }
        long tiemNangUse = 0;
