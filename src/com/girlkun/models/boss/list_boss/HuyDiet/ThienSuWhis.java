...
public class ThienSuWhis extends Boss {
    ...
    public void reward(Player plKill) {
        ...
        int[] manhhiem = new int[]{1066, 1067, 1068, 1069, 1070};
        ...
        if (Util.isTrue(95, 100)) {
            Service.gI().dropItemMap(this.zone, Util.manhTS(zone, manhthuong[randomAWJ], 30, this.location.x, this.location.y, plKill.id));
        } else {
            Service.gI().dropItemMap(this.zone, Util.manhTS(zone, manhhiem[randomGR], 30, this.location.x, this.location.y, plKill.id));
        }
        } else {
    ...
    public double injured(Player plAtt, double damage, boolean piercing, boolean isMobAttack) {
        if (Util.isTrue(20, 50) && plAtt != null) {//tỉ lệ hụt của thiên sứ
